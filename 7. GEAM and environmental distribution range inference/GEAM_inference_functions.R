...
# 6. "GEAM_extremos_plot" function to plot environmental distribution 
# range for each environmental variable (rad/th/ele) and genomic database (G1EAM/G2EAM)
# Depurar un poco la función. No funciona perfectamente bien.
# Por ejemplo, en elevación no hay NAs. No se si los mapas son correctos.
GEAM_extremos_plot <- function(ras, label, env.label){
  ras <- rele
  label <- "G2EAM_ele"
  env.label <- "ele"
  
  tiff(paste(env.label, ".tiff", sep=""), units="in", width=15, height=9, res=300)
  plot(ras, main="Elevation")
  dev.off()
  
  # Comparar los valores extremos de los rasteres con los
  # obtenidos por los GEAMS y entonces decidir:
  
  # 1. Si quedan dentro de los márgenes del raster ambiental (al menos de uno)
  # Se hace el plot
  ...
  if(min(ext_pred) > min_ras | max(ext_pred) < max_ras){
    ...
    # Cambiar a 1000 los NAs de los rasteres (NA=mar) para
    # poder operar con ellos
    values(ras)[which(is.na(values(ras)))] <- 1000
    
    values(ras) <- round(values(ras), digits=6)
    presencias <- which(values(ras) >= min(ext_pred) & values(ras) <= max(ext_pred))
    
    # Si se predice algo en el mar, quitarlo:
    if(any(presencias %in% which(values(ras) == 1000))){
      presencias <- presencias[-which(presencias %in% which(values(ras)==0))]
    }
    values(ras)[presencias] <- 950
    ...
    # Hacer y guardar mapas
    x11()
    plot(ras, breaks=cuts, col = pal(3), legend=T, 
         main=label, xlab=paste("from", round(ext_pred[1,1], digits=2), 
  ...
  }else{ # 2. Si se salen, (=la predicción ocupa todo el ambiente),
    # el mapa no tiene sentido (=todo). Se dice en un mensaje
